...
// Function to check if a message is a command (without prefix)
function isCommand(message) {
    ...
    return commands.includes(words[0]);
}
...
async function handleMessages(sock, messageUpdate, printLog) {
    try {
        ...
        // Check if message is a command (without prefix)
        const isCmd = isCommand(rawText.toLowerCase());
        ...
        if (isBanned(senderId) && !rawText.toLowerCase().startsWith('unban')) {
            if (Math.random() < 0.1) {
                await sock.sendMessage(chatId, {
        ...
        // Then check for command (without prefix)
        if (!isCmd) {
        ...
        switch (command) {
            ...
            case 'tictactoe':
                ...
                break;
            case 'move':
                const position = parseInt(words[1]);
                if (isNaN(position)) {
            ...
            case 'iss':
                ...
                await stupidCommand(sock, chatId, stupidQuotedMsg, stupidMentionedJid, senderId, words.slice(1));
                commandExecuted = true;
                break;
            case 'dare':
            ...
            case 'take':
            case 'steal':
                await takeCommand(sock, chatId, message, words.slice(1));
                commandExecuted = true;
                break;
            case 'flirt':
            ...
            case 'clearsesi':
                ...
                break;
            case 'autostatus':
                await autoStatusCommand(sock, chatId, message, words.slice(1));
                commandExecuted = true;
                break;
            case 'metallic':
            ...
            case 'heart':
                ...
                break;
            case 'horny':
                await miscCommand(sock, chatId, message, ['horny', ...words.slice(1)]);
                commandExecuted = true;
                break;
            case 'circle':
                await miscCommand(sock, chatId, message, ['circle', ...words.slice(1)]);
                commandExecuted = true;
                break;
            case 'lgbt':
                await miscCommand(sock, chatId, message, ['lgbt', ...words.slice(1)]);
                commandExecuted = true;
                break;
            case 'lolice':
                await miscCommand(sock, chatId, message, ['lolice', ...words.slice(1)]);
                commandExecuted = true;
                break;
            case 'simpcard':
                await miscCommand(sock, chatId, message, ['simpcard', ...words.slice(1)]);
                commandExecuted = true;
                break;
            case 'tonikawa':
                await miscCommand(sock, chatId, message, ['tonikawa', ...words.slice(1)]);
                commandExecuted = true;
                break;
            case 'its-so-stupid':
                await miscCommand(sock, chatId, message, ['its-so-stupid', ...words.slice(1)]);
                commandExecuted = true;
                break;
            case 'namecard':
                await miscCommand(sock, chatId, message, ['namecard', ...words.slice(1)]);
                commandExecuted = true;
                break;
            case 'oogway2':
            case 'oogway':
                const sub = command === 'oogway2' ? 'oogway2' : 'oogway';
                await miscCommand(sock, chatId, message, [sub, ...words.slice(1)]);
                commandExecuted = true;
                break;
            case 'tweet':
                await miscCommand(sock, chatId, message, ['tweet', ...words.slice(1)]);
                commandExecuted = true;
                break;
            case 'ytcomment':
                await miscCommand(sock, chatId, message, ['youtube-comment', ...words.slice(1)]);
                commandExecuted = true;
                break;
            case 'comrade':
            case 'gay':
            ...
            case 'passed':
            case 'triggered':
                await miscCommand(sock, chatId, message, [command, ...words.slice(1)]);
                commandExecuted = true;
                break;
            case 'animu':
                await animeCommand(sock, chatId, message, words.slice(1));
                commandExecuted = true;
                break;
            case 'nom':
            case 'poke':
            ...
            case 'crop':
                ...
                break;
            case 'pies':
                await piesCommand(sock, chatId, message, words.slice(1));
                commandExecuted = true;
                break;
            case 'china':
            ...
            case 'hijab':
                ...
                break;
            case 'update':
                const zipArg = words[1] && words[1].startsWith('http') ? words[1] : '';
            ...
            case 'rmbg':
            case 'nobg':
                await removebgCommand.exec(sock, message, words.slice(1));
                commandExecuted = true;
                break;
            case 'remini':
            case 'enhance':
            case 'upscale':
                await reminiCommand(sock, chatId, message, words.slice(1));
                commandExecuted = true;
                break;
            case 'sora':
