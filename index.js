...
let phoneNumber = "255760774888"
let owner = JSON.parse(fs.readFileSync('./data/owner.json'))

global.botname = "VORTEX-XMD"
global.themeemoji = "•"
...
async function startHansTechInc() {
    ...
    HansTechInc.ev.on('messages.upsert', async chatUpdate => {
        try {
            ...
            // Note: HansTechInc.public is not synced, so we check mode in main.js instead
    ...
    // Handle pairing code
    if (pairingCode && !HansTechInc.authState.creds.registered) {
        ...
        if (!!global.phoneNumber) {
            ...
            phoneNumber = await question(chalk.bgBlack(chalk.greenBright(`Please type your WhatsApp number 😍\nFormat: 25576077XXXX (without + or spaces) : `)))
        }
    ...
    HansTechInc.ev.on('connection.update', async (s) => {
        ...
        if (connection == "open") {
            ...
            console.log(chalk.magenta(`\n${global.themeemoji || '•'} YT CHANNEL: HANSTZTECH`))
            console.log(chalk.magenta(`${global.themeemoji || '•'} GITHUB: HansTz`))
